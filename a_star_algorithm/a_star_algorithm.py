...
from constants import DIRECTIONS, OBSTACLE, ROBOT
...
def print_array(new_grid):
    for row in range(new_grid.shape[0]):
        for col in range(new_grid.shape[1]):
            if new_grid[row, col] == ROBOT:
                c = "M"
            elif new_grid[row, col] == OBSTACLE:
                c = "*"
            else :
                c = "_"
            print(c, end="")
...
def print_steps(grid, path):
    for ind, step in enumerate(path):
        print(f"step {ind}:")
        new_grid = grid.copy()
        new_grid[step] = ROBOT
        print_array(new_grid)
